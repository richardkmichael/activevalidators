...
module ActiveModel
  module Validations
    def self.activevalidators
      %w(image email url respond_to phone slug ip credit_card date password twitter postal_code tracking_number)
    end
    ...
    module HelperMethods
      ActiveModel::Validations.activevalidators.each do | validator |
        define_method('validates_' + validator) do | *fields |
          # Remove the first hash from 'fields' and set options to it ; or true, if none.
